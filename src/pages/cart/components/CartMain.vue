...
const sum = computed(() => {
  //计算选中商品的数量
  const cartArr = cartList.value.filter((item) => item.selected)
...
const sumAccount = computed(() => {
  const cartArr = cartList.value.filter((item) => item.selected)
  return cartArr.reduce((sum, item) => {
    sum = item.count * item.nowPrice + sum
    return sum
  }, 0)
})
//修改选中状态
const changeSelect = (item: CartItem) => {
...
const changeSelectState = () => {
  cartList.value.map((item) => {
    item.selected = isAllChecked.value
  })
  cartList.value.map((item) => {
    item.selected = !item.selected
  })
  putMemberCartSelectedApi(isAllChecked.value.toString())
}
// 修改商品数量
...
const deleteItem = (item: CartItem) => {
  uni.showModal({
    ...
    success: async (success) => {
      if (success.confirm) {
        const res = await deleteMemberCart({ ids: [item.skuId] })
...
<template>
  <scroll-view scroll-y class="scroll-view" @scrolltolower="onScrollToLower">
    ...
    <template v-if="memberStore.profile">
          ...
          <uni-swipe-action-item v-for="item in cartList" :key="item.skuId" class="cart-swipe">
            ...
            <template #right>
              <view class="cart-swipe-right">
                <button class="button delete-button" @tap="deleteItem(item)">删除</button>
              </view>
            </template>
      ...
      <!-- 吸底工具栏 -->
      <view class="toolbar" v-if="cartList?.length > 0">
        <text class="all" :class="{ checked: isAllChecked }" @tap="changeSelectState">全选</text>
        <text class="text">合计:</text>
        <text class="amount">{{ sumAccount.toFixed(2) }}</text>
        <view class="button-grounp">
          <view class="button payment-button" :class="{ disabled: false }" @tap="goPay">
            去结算({{ sum }})
          </view>
        </view>
      </view>
