...
//滑动修改容器
const changeTab = (ev: UniHelper.SwiperOnChangeEvent) => {
...
<template>
  <view class="viewport">
    ...
    <!-- 滑动容器 -->
    <swiper class="swiper" @change="changeTab" :current="activeIndex">
      <!-- 滑动项 -->
      <swiper-item v-for="item in orderTabs" :key="item.orderState">
        <OrderList :order-state="item.orderState" v-if="item.isRender" />
      </swiper-item>
    </swiper>
  </view>
</template>
